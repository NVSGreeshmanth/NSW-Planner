...
# ── Local time ──────────────────────────────────────────────────────────────
# TfNSW interprets itdDate/itdTime as Sydney local time. Hosts like Render run in
# UTC, so datetime.now() would be ~10–11h behind Sydney and the Departure Monitor
# would return services from hours ago. Always anchor to Australia/Sydney (DST-aware).
try:
...
def now_syd():
    if SYD:
        return datetime.datetime.now(SYD)
    # Fallback if the tz database is missing: fixed AEST (+10). Misses DST by 1h
    # but is far better than raw UTC.
    ...
    def do_GET(self):
        ...
        # ── Trip Planner ─────────────────────────────────────────────────────
            ...
            if qs.get("itdDate") and qs.get("itdTime"):
                ...
                if offset_mins > 0:
                    t_offset = now_syd() - datetime.timedelta(minutes=offset_mins)
        ...
        # ── Departure Monitor ─────────────────────────────────────────────────
            ...
            now = now_syd() - datetime.timedelta(minutes=offset_mins)
            params = {
                "outputFormat":          "rapidJSON",
                "coordOutputFormat":     "EPSG:4326",
                "mode":                  "direct",
